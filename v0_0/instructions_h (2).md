...
All levels use **multiple-choice buttons**. No numpad, no keyboard input — tap only. The number of choices scales with level difficulty:

| Levels | Choices | Why |
|--------|---------|-----|
| 1–2 | 3 | Young children need fewer options to stay confident. |
| 3–4 | 4 | Comfortable range for growing skills. |
| 5–6 | 5 | More plausible distractors as operations get harder. |
| 7+ | 6 | Full challenge — requires real mental math to distinguish. |

**Critical: debounce / lock answers.** Once a child taps an answer, all buttons must be **immediately disabled** until the next question is rendered. This prevents rapid double-taps from counting as multiple correct answers.
...
| Level | Name | New Operation | Number Range | Accumulated Ops | Choices | Slime Speed | Problems to Clear |
|-------|------|--------------|--------------|-----------------|---------|-------------|-------------------|
| 1 | "Baby Steps" | `a + b` | 1–9 (single digit) | `+` | 3 | Very slow (12 s) | 5 |
| 2 | "Take Away" | `a − b` | 1–9, result ≥ 0 | `+`, `−` | 3 | Very slow (12 s) | 8 |
| 3 | "Double Digits" | `a + b` (range expands) | 0–20 | `+`, `−` | 4 | Slow (10 s) | 8 |
| 4 | "Subtract & Stretch" | `a − b` (range expands) | 0–20, result ≥ 0 | `+`, `−` | 4 | Slow (10 s) | 10 |
| 5 | "First Times" | `a × b` | a, b ∈ 0–5 | `+`, `−`, `×` | 5 | Medium (8 s) | 10 |
| 6 | "Fair Shares" | `a ÷ b` | clean division only (e.g. 4÷2, 12÷3, 25÷5) | `+`, `−`, `×`, `÷` | 5 | Medium (8 s) | 10 |
| 7 | *TBD — see §5.4* | *TBD* | *TBD* | all prior + new | 6 | Medium-fast (7 s) | 12 |
| 8 | *TBD* | *TBD* | *TBD* | all prior + new | 6 | Medium-fast (7 s) | 12 |
| 9 | *TBD* | *TBD* | *TBD* | all prior + new | 6 | Fast (6 s) | 15 |
| 10 | *TBD* | *TBD* | *TBD* | all prior + new | 6 | Fast (6 s) | 15 |
| 11 | *TBD* | *TBD* | *TBD* | all prior + new | 6 | Very fast (5 s) | 18 |
| 12 | "Speed Demon" | all ops, full range | mixed, age-appropriate | all | 6 | Very fast (5 s) | 20 |
...
- Input: 3 multiple-choice buttons
...
- Input: 4 multiple-choice buttons
...
- Input: 5 multiple-choice buttons
...
### 5.6 Distractor Generation (All Levels)

All levels use multiple choice. The number of choices scales per §3.2. Generate wrong answers that are *plausible*:
- First distractor = correct answer ± 1
- Second distractor = correct answer ± 2 or ± 3
- Additional distractors (for 5–6 choice levels) = correct answer ± 4 to ± 6
- Never show negative numbers as choices.
- Never show duplicate choices.
- Shuffle the position of the correct answer.
- For division/multiplication, prefer distractors that are in the same "family" (e.g. nearby multiples).
...
function generateProblem(level) {
  ...
  // 4. Determine number of choices based on level (3/4/5/6 per §3.2)
  // 5. Return { text: "3 + 4", answer: 7, choices: [5, 7, 6] }
  //    Division: always generate via (b * result) ÷ b to ensure clean answers
  //    Subtraction: always ensure a ≥ b (no negatives)
}
```

### 5.8 Answer Lock (Anti-Spam)

When a child taps any answer button:
1. **Immediately disable all buttons** (set `pointer-events: none` or a global `answering` lock flag).
2. Process the answer (correct → pop slime + score; wrong → shake + flash).
3. After the feedback delay (~300ms), generate the next question and **re-enable buttons**.

This prevents the rapid-tap exploit where mashing one button counts as multiple correct answers.
...
- Same dark "Space Arcade" aesthetic as the rest of the game.
- Rows alternate in subtle dark tones (navy / slightly lighter navy).
...
### 7.1 Aesthetic: "Space Arcade"

Dark, glowing, atmospheric — like a friendly arcade in space. The dark background makes the colorful slimes pop. Rounded corners on everything, generous spacing, big tap targets.
...
| Game area background | Darker navy | `#16213e` |
| Game area border | Midnight blue | `#0f3460` |
| Text / UI | Light grey | `#eeeeee` |
| Subdued text | Mid grey | `#cccccc` |
| Title / accents | Lime green | `#7fff00` |
| Slime 1 | Green | `#4ade80` |
| Slime 2 | Blue | `#60a5fa` |
| Slime 3 | Pink | `#f472b6` |
| Slime 4 | Orange | `#fb923c` |
| Slime 5 | Purple | `#a78bfa` |
| Slime 6 | Teal | `#34d399` |
| Correct flash | Green glow | `#2ecc40` |
| Wrong flash | Red | `#e74c3c` |
| Buttons | Midnight blue | `#0f3460` border `#1a4a80` |
| Button hover | Brighter blue | `#1a5296` |

### 7.3 Typography

- **Primary font:** `'Segoe UI', sans-serif` — clean, readable, available everywhere.
- **Numbers in problems:** Same font, bold, large size (48px+).
- Fallback: system sans-serif stack.

### 7.4 Slime Design (Canvas-rendered)

Slimes are drawn on a `<canvas>` element with:
- A rounded blob body shape (Bézier curves)
- Two white eyes with colored pupils
- A small smile arc
- Three drip circles underneath
- Idle wobble animation via `requestAnimationFrame`
...
│                                  │
│                                  │
│  ┌─────┐ ┌─────┐ ┌─────┐        │  ← Answer choice buttons (3–6 depending on level)
│  │  5  │ │  7  │ │  6  │        │
│  └─────┘ └─────┘ └─────┘        │
└──────────────────────────────────┘
```
...
```js
function generateProblem(level) {
  // Returns { text: "3 + 4", answer: 7, choices: [5, 7, 6] }
  // Number of choices determined by level (3/4/5/6 per §3.2)
...
- Answer buttons: minimum `48px × 48px` tap targets (WCAG compliant).
...
4. Multiple choice input with scaling choices (3/4/5/6 per level).
5. Answer lock / debounce to prevent rapid-tap exploit.
